package fail.toepic.recycleroverview.basic
...
import fail.toepic.recycleroverview.basic_simple.BasicSimpleFragment
import fail.toepic.recycleroverview.basic_simple.BasicSimpleFragmentDirections
...
class BasicFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = BaseListAdapter(delegate)
            ...
            setRecyclerView(
                list,
                LinearLayoutManager(this@BasicFragment.context, RecyclerView.VERTICAL, false),
                adapter ?: BaseListAdapter(delegate)
            )
        //TODO 어거먼트를 이용한 추상화 고려 할 것
        adapter?.submitList(MenuStructureRepository.getList("BASIC"))
...
val delegate : BaseAdapterDelegate<Item, Holder> = object :
    ...
    override val eventLisner: (key: String, Holder: Holder,item : Item) -> Unit? = {
        key, holder, item ->
        when(item.name){
            "simple" -> holder.itemView.findNavController().navigate(NavGraphDirections.moveBasicSimple())
        }
        Unit
    }
